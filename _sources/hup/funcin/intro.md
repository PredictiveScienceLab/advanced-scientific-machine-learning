...
Many uncertainty-propagation methods require a finite vector of uncertain inputs. A coefficient field, initial condition, boundary condition, or geometry is instead a function, so its discretization may be too large to use as a direct surrogate-model input.
